...
func (h *ExerciseHandler) ExerciseStatistic(c echo.Context) (err error) {
	...
	group := bson.D{
		primitive.E{Key: "_id", Value: bson.D{
			primitive.E{Key: "$dateToString", Value: bson.D{
				primitive.E{Key: "format", Value: "%Y-%m-%d"},
	...
	ctx := context.TODO()
	totalInfoCursor, err := exerciseCollection.Aggregate(ctx, mongo.Pipeline{matchStage, groupStage})
	if err != nil {
