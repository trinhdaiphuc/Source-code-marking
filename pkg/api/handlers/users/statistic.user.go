...
func (h *UserHandler) UserStatistic(c echo.Context) (err error) {
	...
	match := bson.D{
		...
		primitive.E{Key: "role", Value: statistic.UserRole},
	}
	...
	bookPermissionCollection := models.GetUserCollection(h.DB)
	ctx := context.TODO()
	totalInfoCursor, err := bookPermissionCollection.Aggregate(ctx, mongo.Pipeline{matchStage, groupStage})
	if err != nil {
